...
function selectcategory_data()
{
    ...
    $stmt = $conn->prepare("SELECT DISTINCT category FROM menus WHERE state = '上架'");
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $mydata = array();
...
function selectupcategory_data()
{

    $conn = create_connection();

    $stmt = $conn->prepare("SELECT DISTINCT category FROM menus WHERE status = '上架'");
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $mydata = array();
